...
func MethodIndexerCallback(orm *gorm.DB, transferMethodTracker TransferMethodTracker, from, to int, contractAddress string, unmarshalSDKWrapper *UnmarshalSDKWrapper) error {
	errs, _ := errgroup.WithContext(context.Background())
		...
		errs.Go(func() error {
			return func(j int) error {
				...
				case GetTransferMethodHash():
					...
					transferMethodTracker.mutex.Lock()
					transferMethods = append(transferMethods, method)
					transferMethodTracker.mutex.Unlock()
		...
		for true {
			...
			for i := 0; i < parallelCalls; i++ {
				...
				go func(routineFrom, routineTo int) {
					...
					err = MethodIndexerCallback(orm, transferMethodTracker, routineFrom, routineTo, config.ContractAddress, unmarshalSDKWrapper)
					if err != nil {
